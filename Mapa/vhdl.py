...
def calcular_paquete(secciones):
    ...
    print ("Paquete > CVS : {} | SEM : {} | PAN : {} | MDC : {}".format(n_vias,n_semaforos,n_pans,int(n_cambios/2)))
    
    return [n_vias,n_semaforos,n_pans,int(n_cambios/2)]
#%%
...
def creando_wrapper(secciones,objetos):        
    ...
    instanciar_separador(f,separador)
    
    instanciar_mediador(f,mediador)
...
#%%   
def instanciar_separador(f,nombre):   
...
#%%   
def instanciar_mediador(f,nombre):   
...
def creando_red(secciones,objetos,tabla):        
    ...
    for i in range(N_CVS):
        ...
        if secciones[i].cambio:
            ...
            #print ("Cambio nodo : {}".format(i+1))
            # Si > :
            if (secciones[i].desvio_inf_dir == ">" or secciones[i].desvio_sup_dir == ">"):
                print ("Nodo : {} > Cambio".format(i+1))
                f.write("\t\t"+"Estado_post => mdc_ante_o_"+str(cambio_index)+",\n")
            ...
            # else
                ...
                if secciones[i].posterior != "":
                    ...
                    print ("Nodo : {} > {}".format(i+1,posterior))   
                    f.write("\t\t"+"Estado_post => conector_"+str(posterior)+",\n")
                    
             # Si < :
            if (secciones[i].desvio_inf_dir == "<" or secciones[i].desvio_sup_dir == "<"):
                print ("Nodo : Cambio < {}".format(i+1))
                f.write("\t\t"+"Estado_ante => mdc_desv_o_"+str(cambio_index)+",\n")
                cambios_vecinos.append(i+1)
                #print ("ACA {}".format(cambios_vecinos))
                # Post = cambio
            # else
        ...
        # Si no soy cambio pero mi vecino lo es
        if (secciones[i].cambio == False): 
            
            # Vecino anterior
            if secciones[i].anterior != "":
                anterior = secciones[i].anterior
                if (secciones[anterior-1].tipo == "Cruce"):
                    if (secciones[anterior-1].desvio_inf_dir == ">" or secciones[anterior-1].desvio_sup_dir == ">"):
                        print ("Nodo : cambio < {}".format(i+1))
           ...
           # Vecino posterior
            if secciones[i].posterior != "":
                posterior = secciones[i].posterior
                if (secciones[posterior-1].tipo == "Cruce"):
                    if (secciones[posterior-1].desvio_inf_dir == "<" or secciones[posterior-1].desvio_sup_dir == "<"):
                        print ("Nodo : {} > cambio".format(i+1))
                    else:
                        print ("Nodo : {} > {}".format(i+1,posterior))
                        f.write("\t\t"+"Estado_post => conector_"+str(posterior)+",\n")
                else :
                    print ("Nodo : {} > {}".format(i+1,secciones[i].posterior))
                    f.write("\t\t"+"Estado_post => conector_"+str(posterior)+",\n")  
    ...
    for i in range(N_MDC):
        ...
        f.write("\t\t"+"Estado_ante_i => conector_"+str(cambios_vecinos[0])+",\n")
        f.write("\t\t"+"Estado_ante_o => mdc_ante_o_"+str(i+1)+",\n")
        f.write("\t\t"+"Estado_post_i => conector_"+str(cambios_vecinos[1])+",\n")
        f.write("\t\t"+"Estado_post_o => mdc_post_o_"+str(i+1)+",\n")
        f.write("\t\t"+"Estado_desv_i => conector_"+str(cambios_vecinos[2])+",\n")
